        ...
        <div className="hidden lg:flex gap-5 text-xl">
          {SECTIONS.map((section) => (
            <Link
              key={section}
              href={`#${section}`}
              className={`${activeSection == section ? "underline text-orange-500" : ""} hover:text-orange-800 hover:underline`}
            >
              {section}
            </Link>
          ))}
              ...
              <div className={`${openMenu ? "block" : "hidden"} w-40 absolute -right-1 top-7 bg-gray-300 rounded flex flex-col gap-1 p-2`}>
                {SECTIONS.map((section) => (
                  <Link
                    ...
                    href={`#${section}`}
                    className="bg-gray-600 text-orange-400 rounded py-0.5 px-2"
                  >
                    {section}
                  </Link>
                ))}
